...
export default function Contact() {
  ...
  return (
    <div style={{ background: '#F5F2EC', fontFamily: 'var(--font-playfair)', paddingTop: '88px' }}>
      ...
      <section style={{ padding: '80px', background: '#F5F2EC' }}>
        <div style={{ maxWidth: '1100px', margin: '0 auto', display: 'grid', gridTemplateColumns: '1fr 1.4fr', gap: '80px', alignItems: 'start' }}>
          ...
          <div>
            ...
            <h2 style={{ fontFamily: 'var(--font-playfair)', fontSize: '36px', fontWeight: '700', color: '#1a1a1a', letterSpacing: '-1px', lineHeight: '1.1', marginBottom: '20px' }}>
              We read every message personally.
            </h2>
            <p style={{ fontFamily: 'var(--font-inter)', fontSize: '15px', color: '#6a6060', lineHeight: '1.85', marginBottom: '40px' }}>
            ...
            <div style={{ marginBottom: '32px' }}>
              ...
              <a href="mailto:hello@flowcapitals.com" style={{ fontFamily: 'var(--font-playfair)', fontSize: '20px', fontWeight: '700', color: '#2B5EA7', textDecoration: 'none', transition: 'color 0.2s' }}
                onMouseEnter={e => e.currentTarget.style.color = '#1a4a8f'}
                onMouseLeave={e => e.currentTarget.style.color = '#2B5EA7'}
              >
            ...
            {/* Response time */}
            <div style={{ padding: '20px 24px', background: '#ffffff', borderLeft: '3px solid #2B5EA7', marginBottom: '32px' }}>
            ...
            {/* What to expect */}
            <div>
              <div style={{ fontFamily: 'var(--font-inter)', fontSize: '9px', color: '#8a8070', letterSpacing: '0.12em', textTransform: 'uppercase' as const, marginBottom: '16px' }}>Common questions we answer</div>
              {[
          ...
          {/* Right — form */}
          <div style={{ background: '#ffffff', padding: '48px 44px', border: '0.5px solid rgba(26,26,26,0.08)', boxShadow: '0 4px 40px rgba(0,0,0,0.04)' }}>
            {sent ? (
              ...
              <>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px', marginBottom: '16px' }}>
                  <div>
                    ...
                    <input type="text" value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} placeholder="Mauro Steenhoudt" style={{ width: '100%', background: '#F5F2EC', border: '0.5px solid rgba(26,26,26,0.1)', borderRadius: '8px', padding: '11px 14px', fontFamily: 'var(--font-inter)', fontSize: '13px', color: '#1a1a1a', outline: 'none', transition: 'border-color 0.2s' }}
                  ...
                  <div>
                    ...
                    <input type="email" value={form.email} onChange={e => setForm({ ...form, email: e.target.value })} placeholder="you@example.com" style={{ width: '100%', background: '#F5F2EC', border: '0.5px solid rgba(26,26,26,0.1)', borderRadius: '8px', padding: '11px 14px', fontFamily: 'var(--font-inter)', fontSize: '13px', color: '#1a1a1a', outline: 'none', transition: 'border-color 0.2s' }}
                      onFocus={e => e.target.style.borderColor = '#2B5EA7'}
                      onBlur={e => e.target.style.borderColor = 'rgba(26,26,26,0.1)'}
                    />
                  </div>
                ...
                <div style={{ marginBottom: '16px' }}>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px' }}>
                    {subjects.map(s => (
                      <button key={s.value} onClick={() => setForm({ ...form, subject: s.value })} style={{ padding: '10px 14px', background: form.subject === s.value ? 'rgba(43,94,167,0.06)' : '#F5F2EC', border: `0.5px solid ${form.subject === s.value ? '#2B5EA7' : 'rgba(26,26,26,0.1)'}`, borderRadius: '8px', cursor: 'pointer', textAlign: 'left' as const, transition: 'all 0.2s' }}>
                ...
                <div style={{ marginBottom: '24px' }}>
                  ...
                  <textarea value={form.message} onChange={e => setForm({ ...form, message: e.target.value })} rows={5} placeholder="Tell us what you want to know..." style={{ width: '100%', background: '#F5F2EC', border: '0.5px solid rgba(26,26,26,0.1)', borderRadius: '8px', padding: '11px 14px', fontFamily: 'var(--font-inter)', fontSize: '13px', color: '#1a1a1a', outline: 'none', resize: 'vertical' as const, transition: 'border-color 0.2s' }}
                    onFocus={e => e.target.style.borderColor = '#2B5EA7'}
                    onBlur={e => e.target.style.borderColor = 'rgba(26,26,26,0.1)'}
                  />
                </div>

                <button onClick={() => { if (form.name && form.email && form.message) setSent(true) }} style={{ width: '100%', background: '#2B5EA7', color: '#ffffff', fontFamily: 'var(--font-inter)', fontSize: '12px', letterSpacing: '0.1em', textTransform: 'uppercase' as const, padding: '14px', border: 'none', cursor: 'pointer', borderRadius: '8px', fontWeight: '700', transition: 'all 0.2s' }}
      ...
      {/* DARK QUOTE */}
      <section style={{ padding: '80px', background: '#0d1e36', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <div style={{ maxWidth: '600px', textAlign: 'center' as const }}>
          <div style={{ width: '40px', height: '3px', background: '#2B5EA7', margin: '0 auto 32px' }} />
          <p style={{ fontFamily: 'var(--font-playfair)', fontStyle: 'italic', fontSize: '22px', color: '#ffffff', lineHeight: '1.65', marginBottom: '20px' }}>
