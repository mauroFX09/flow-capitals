...
function RoadmapChart({ stages }: { stages: typeof ROADMAP }) {
      ...
      <div style={{ position: 'sticky', top: 0, height: '100vh', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '100%', background: '#0d1e36', padding: '60px', position: 'relative', overflow: 'hidden' }}>
          ...
          <svg ref={null} viewBox={`0 0 ${W} ${H}`} style={{ width: '100%', maxWidth: '380px', height: 'auto', overflow: 'visible', position: 'relative', zIndex: 1 }}>
            ...
            {[0, 1, 2, 3, 4].map(i => (
              <line key={i}
                x1={padX} y1={padY + (i / 4) * (H - padY * 2)}
                x2={W - padX} y2={padY + (i / 4) * (H - padY * 2)}
                stroke="rgba(122,174,232,0.05)" strokeWidth="1" strokeDasharray="4,6"
              />
            ))}
                ...
                <g key={i}>
                  ...
                  {isActive && (
                    <circle cx={pt.x} cy={pt.y} r="20"
                      fill="rgba(122,174,232,0.06)"
                      stroke="rgba(122,174,232,0.15)"
                      strokeWidth="1"
                    />
                  )}
                  {/* Dot shadow */}
                  ...
                  <circle cx={pt.x} cy={pt.y}
                    r={isActive ? 9 : reached ? 6 : 4}
                    fill={reached ? (isActive ? '#7aaee8' : '#2B5EA7') : 'rgba(255,255,255,0.1)'}
                    stroke={reached ? (isActive ? '#ffffff' : '#7aaee8') : 'rgba(122,174,232,0.3)'}
                    strokeWidth={isActive ? '2' : '1.5'}
                    style={{ transition: 'all 0.4s ease' }}
                  />
                  {/* Stage number inside dot */}
                  {reached && (
                    <text x={pt.x} y={pt.y + 1}
                      textAnchor="middle" dominantBaseline="middle"
                      fontFamily="var(--font-inter)" fontSize={isActive ? '8' : '6'}
                      fontWeight="700" fill="#ffffff"
                    >
                      {i + 1}
                    </text>
                  )}
                  {/* Small label above/below for reached dots */}
                  {reached && !isActive && (
                    <text
                      x={pt.x}
                      y={i % 2 === 0 ? pt.y - 14 : pt.y + 18}
                      textAnchor="middle"
                      fontFamily="var(--font-inter)"
                      fontSize="8"
                      fill="rgba(122,174,232,0.5)"
                    >
                      {stages[i].title.split(' ')[0]}
                    </text>
                  )}
                </g>
              )
            ...
            {/* Animated dot at current position on the line */}
        ...
        <div style={{ padding: '60px 64px', height: '100%', display: 'flex', flexDirection: 'column' as const, justifyContent: 'center', background: '#ffffff', overflowY: 'auto' as const }}>
          <div style={{ fontFamily: 'var(--font-inter)', fontSize: '9px', color: '#2B5EA7', letterSpacing: '0.16em', textTransform: 'uppercase' as const, marginBottom: '12px', display: 'flex', alignItems: 'center', gap: '8px' }}>
            ...
            Stage {active.num} of 07
          </div>

          <h3 style={{ fontFamily: 'var(--font-playfair)', fontSize: '38px', fontWeight: '700', color: '#1a1a1a', letterSpacing: '-1px', lineHeight: 1.1, marginBottom: '8px' }}>
            {active.title}
          </h3>
          <p style={{ fontFamily: 'var(--font-playfair)', fontStyle: 'italic', fontSize: '15px', color: '#2B5EA7', marginBottom: '28px' }}>
            {active.sub}
          </p>
    ...
    <div style={{ background: '#F5F2EC', fontFamily: 'var(--font-playfair)', paddingTop: '88px' }}>
      ...
      <section style={{ background: '#F5F2EC' }}>
        <div style={{ padding: '80px 80px 40px' }}>
          <h2 style={{ fontFamily: 'var(--font-playfair)', fontSize: '36px', fontWeight: '700', color: '#1a1a1a', marginBottom: '8px' }}>7-Stage Roadmap</h2>
        ...
        <RoadmapChart stages={ROADMAP} />
      </section>

      {/* WEEKLY SCHEDULE */}
      <section style={{ padding: '80px', background: '#ffffff' }}>
        <div style={{ maxWidth: '1100px', margin: '0 auto' }}>
          <h2 style={{ fontFamily: 'var(--font-playfair)', fontSize: '36px', fontWeight: '700', color: '#1a1a1a', marginBottom: '8px' }}>Weekly Schedule</h2>
          <p style={{ fontFamily: 'var(--font-playfair)', fontStyle: 'italic', fontSize: '15px', color: '#8a8070', marginBottom: '36px' }}>Standard members get 2 live sessions per week. Premium members get 4–5 live sessions plus a personal 1-on-1 call every week.</p>
        ...
        <div style={{ maxWidth: '1100px', margin: '0 auto' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '2px', background: 'rgba(26,26,26,0.06)' }}>
            <div style={{ background: '#ffffff', padding: '48px 44px' }}>
              ...
              <p style={{ fontFamily: 'var(--font-inter)', fontSize: '14px', color: '#6a6060', lineHeight: '1.85', marginBottom: '24px' }}>As a Standard member you are the observer — and that is a position of strength. You move at your own pace, in a context where clarity grows without pressure.</p>
              <p style={{ fontFamily: 'var(--font-inter)', fontSize: '14px', color: '#6a6060', lineHeight: '1.85', marginBottom: '32px' }}>You watch. You learn. You apply. You build your own rhythm. For the self-motivated trader who knows how to take information and work with it independently — this is your path.</p>
            ...
            <div style={{ background: '#0d1e36', padding: '48px 44px' }}>
              ...
              <p style={{ fontFamily: 'var(--font-inter)', fontSize: '14px', color: 'rgba(255,255,255,0.65)', lineHeight: '1.85', marginBottom: '24px' }}>As a Premium member you have a mentor in your corner every single week. Not a recording. Not a chatbot. A real trader who has walked this path.</p>
              <p style={{ fontFamily: 'var(--font-inter)', fontSize: '14px', color: 'rgba(255,255,255,0.65)', lineHeight: '1.85', marginBottom: '32px' }}>The weekly 1-on-1 call is the core of this membership. Every Friday you sit down with your mentor and go through exactly where you are — what is working, what is not, and what to focus on next.</p>
              <div style={{ display: 'flex', flexDirection: 'column' as const, gap: '10px', marginBottom: '32px' }}>
          ...
          <div style={{ marginTop: '2px', background: '#1a1a1a', padding: '32px 44px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <p style={{ fontFamily: 'var(--font-playfair)', fontStyle: 'italic', fontSize: '16px', color: 'rgba(255,255,255,0.6)', margin: 0, lineHeight: '1.6' }}>&ldquo;Not everyone needs a mentor. But everyone who has one moves faster.&rdquo;</p>
            <a href="/membership" style={{ background: '#2B5EA7', color: '#ffffff', fontFamily: 'var(--font-inter)', fontSize: '11px', letterSpacing: '0.12em', textTransform: 'uppercase' as const, padding: '14px 32px', textDecoration: 'none', fontWeight: '700', whiteSpace: 'nowrap' as const, flexShrink: 0, marginLeft: '40px' }}
      ...
      <section style={{ padding: '80px', background: '#F5F2EC' }}>
        <div style={{ maxWidth: '1100px', margin: '0 auto', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '80px', alignItems: 'center' }}>
          <div>
            ...
            <h2 style={{ fontFamily: 'var(--font-playfair)', fontSize: '40px', fontWeight: '700', color: '#1a1a1a', marginBottom: '0', lineHeight: '1.1', letterSpacing: '-1px' }}>
      ...
      <section style={{ padding: '80px', background: '#1a2a4a', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div>
          <h2 style={{ fontFamily: 'var(--font-playfair)', fontSize: '32px', fontWeight: '700', color: '#ffffff', marginBottom: '8px' }}>Ready to follow the blueprint?</h2>
        ...
        <div style={{ display: 'flex', gap: '12px' }}>
          <a href="/membership" style={{ background: '#2B5EA7', color: '#ffffff', fontFamily: 'var(--font-inter)', fontSize: '11px', letterSpacing: '0.12em', textTransform: 'uppercase' as const, padding: '14px 32px', textDecoration: 'none', fontWeight: '700', transition: 'all 0.2s' }}
            ...
            onMouseLeave={e => e.currentTarget.style.background = '#2B5EA7'}
          >View Membership →</a>
          <a href="/discovery" style={{ background: 'none', border: '1px solid rgba(255,255,255,0.25)', color: '#ffffff', fontFamily: 'var(--font-inter)', fontSize: '11px', letterSpacing: '0.12em', textTransform: 'uppercase' as const, padding: '13px 28px', textDecoration: 'none', transition: 'all 0.2s' }}
