    ...
    <div style={{ background: '#F5F2EC', fontFamily: 'var(--font-playfair)', paddingTop: '88px' }}>
      ...
      <section style={{ padding: '80px', background: '#F5F2EC' }}>
        <div style={{ maxWidth: '1100px', margin: '0 auto' }}>
          ...
          <p style={{ fontFamily: 'var(--font-playfair)', fontStyle: 'italic', fontSize: '15px', color: '#8a8070', marginBottom: '40px' }}>From knowing nothing to scaling funded accounts. No guessing. No wasted money.</p>

          {/* Stage tabs */}
          <div style={{ display: 'flex', borderBottom: '1px solid rgba(26,26,26,0.1)', marginBottom: '0' }}>
            {ROADMAP.map((stage, i) => (
              <button key={i} onClick={() => setActiveStage(i)} style={{ flex: 1, padding: '14px 6px', background: 'none', border: 'none', borderBottom: activeStage === i ? '2px solid #2B5EA7' : '2px solid transparent', color: activeStage === i ? '#2B5EA7' : '#8a8070', fontFamily: 'var(--font-inter)', fontSize: '10px', cursor: 'pointer', transition: 'all 0.2s', marginBottom: '-1px', textTransform: 'uppercase' as const, letterSpacing: '0.06em' }}>
          ...
          <div style={{ background: '#ffffff', padding: '56px', border: '0.5px solid rgba(26,26,26,0.08)', borderTop: 'none' }}>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 2fr', gap: '72px', alignItems: 'start' }}>
              <div>
                <div style={{ fontFamily: 'var(--font-playfair)', fontSize: '96px', fontWeight: '700', color: 'rgba(43,94,167,0.07)', lineHeight: 1 }}>{ROADMAP[activeStage].num}</div>
                <h3 style={{ fontFamily: 'var(--font-playfair)', fontSize: '28px', fontWeight: '700', color: '#1a1a1a', margin: '-12px 0 8px', lineHeight: '1.2' }}>{ROADMAP[activeStage].title}</h3>
                <p style={{ fontFamily: 'var(--font-playfair)', fontStyle: 'italic', fontSize: '14px', color: '#2B5EA7', margin: '0 0 28px' }}>{ROADMAP[activeStage].sub}</p>
                ...
                <div style={{ display: 'flex', gap: '10px' }}>
                  <button onClick={() => setActiveStage(Math.max(0, activeStage - 1))} style={{ background: 'none', border: '1px solid rgba(26,26,26,0.15)', color: '#8a8070', fontFamily: 'var(--font-inter)', fontSize: '11px', padding: '8px 18px', cursor: 'pointer', transition: 'all 0.2s' }}
                  ...
                  >← Prev</button>
                  <button onClick={() => setActiveStage(Math.min(6, activeStage + 1))} style={{ background: activeStage === 6 ? 'none' : '#2B5EA7', border: `1px solid ${activeStage === 6 ? 'rgba(26,26,26,0.15)' : '#2B5EA7'}`, color: activeStage === 6 ? '#8a8070' : '#fff', fontFamily: 'var(--font-inter)', fontSize: '11px', padding: '8px 18px', cursor: 'pointer', fontWeight: '600' }}>
                ...
                {ROADMAP[activeStage].points.map((point, i) => {
                  ...
                  return (
                    <li key={i} style={{ display: 'flex', gap: '16px', alignItems: 'flex-start', padding: '14px 0', borderBottom: '0.5px solid rgba(26,26,26,0.06)' }}>
                      ...
                      <span style={{ fontSize: '15px', color: isPremium ? '#2B5EA7' : '#3a3530', lineHeight: '1.65', fontFamily: 'var(--font-playfair)', fontStyle: isPremium ? 'italic' : 'normal', fontWeight: isPremium ? '500' : '400' }}>{point}</span>
                    </li>
                  )
                })}
              </ul>
