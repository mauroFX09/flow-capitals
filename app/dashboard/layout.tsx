  ...
  const ArrivalOverlay = () => (
    <div style={{ position: 'fixed', inset: 0, zIndex: 999, background: '#0d1e36', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div style={{ maxWidth: '560px', width: '100%', padding: '0 32px', textAlign: 'center' as const }}>
        ...
        <button onClick={dismissArrival} style={{ background: '#2B5EA7', color: '#ffffff', fontFamily: 'var(--font-inter)', fontSize: '12px', fontWeight: '700', letterSpacing: '0.12em', textTransform: 'uppercase' as const, padding: '16px 48px', border: 'none', borderRadius: '12px', cursor: 'pointer' }}>Enter the platform →</button>
      </div>
    </div>
  )
  ...
  const MobileTopBar = () => (
    <div style={{ position: 'fixed', top: 0, left: 0, right: 0, zIndex: 60, height: '56px', background: sidebar, borderBottom: `0.5px solid ${border}`, display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 16px' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
        ...
        <a href="/dashboard/profile"><AvatarCircle size={32} /></a>
      </div>
    </div>
  )
    ...
    <>
          ...
          {MORE_ITEMS.filter(item => {
            ...
            return (
              <a key={item.href} href={item.href} style={{ display: 'flex', alignItems: 'center', gap: '14px', padding: '14px 16px', borderRadius: '12px', textDecoration: 'none', background: active ? navActive : 'transparent', marginBottom: '2px' }}>
                ...
                {(item as any).soon && <span style={{ fontSize: '9px', color: accent, background: dark ? 'rgba(122,174,232,0.1)' : 'rgba(43,94,167,0.08)', padding: '2px 8px', borderRadius: '4px', fontFamily: 'var(--font-inter)', marginLeft: 'auto' }}>Soon</span>}
              </a>
            )
          })}
          <div style={{ borderTop: `0.5px solid ${border}`, marginTop: '4px', paddingTop: '8px' }}>
            <button onClick={handleLogout} style={{ width: '100%', background: 'none', border: 'none', display: 'flex', alignItems: 'center', gap: '14px', padding: '14px 16px', borderRadius: '12px', cursor: 'pointer', color: '#dc3232', fontFamily: 'var(--font-inter)', fontSize: '14px' }}>
      ...
      <div style={{ position: 'fixed', bottom: 0, left: 0, right: 0, zIndex: 70, height: '72px', background: sidebar, borderTop: `0.5px solid ${border}`, display: 'flex', alignItems: 'center', justifyContent: 'space-around', padding: '0 8px', paddingBottom: 'env(safe-area-inset-bottom)' }}>
        {NAV_ITEMS.filter(item => {
          if (item.access === 'premium' && !isPremium) return true // show but greyed
          return true
        }).map(item => {
          ...
          return (
            <button key={item.href}
              onClick={() => {
                ...
                if (locked) { router.push('/dashboard/upgrade'); return }
                router.push(item.href)
              }}
              style={{ display: 'flex', flexDirection: 'column' as const, alignItems: 'center', gap: '4px', background: 'none', border: 'none', cursor: 'pointer', padding: '8px 12px', borderRadius: '12px', flex: 1, opacity: locked ? 0.5 : 1, position: 'relative' }}
            >
            ...
            {[...NAV_ITEMS.filter(i => i.href !== '__more'), ...MORE_ITEMS].filter(item => {
              ...
              return (
                ...
                >
                  ...
                  {!collapsed && soon && <span style={{ fontSize: '9px', color: accent, background: dark ? 'rgba(122,174,232,0.1)' : 'rgba(43,94,167,0.08)', padding: '1px 6px', borderRadius: '4px', fontFamily: 'var(--font-inter)' }}>Soon</span>}
                </a>
              )
            })}
          ...
          <div style={{ padding: collapsed ? '12px 8px' : '14px', borderTop: `0.5px solid ${border}` }}>
            {!collapsed ? (
              ...
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '8px' }}>
                <button onClick={() => { const n = !dark; setDark(n); localStorage.setItem('fc-dark-mode', n.toString()); window.dispatchEvent(new Event('fc-theme-change')) }} style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: '16px' }}>{dark ? '🌙' : '☀️'}</button>
                <a href="/dashboard/profile"><AvatarCircle size={30} /></a>
