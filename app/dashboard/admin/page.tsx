    ...
    <div style={{ padding: '40px 48px', background: bg, minHeight: '100vh' }}>
      ...
      <div style={{ marginBottom: '28px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between' }}>
          ...
          <button onClick={() => setShowForm(!showForm)} style={{ background: accent, color: '#ffffff', fontFamily: 'var(--font-inter)', fontSize: '11px', letterSpacing: '0.1em', textTransform: 'uppercase' as const, padding: '10px 20px', border: 'none', cursor: 'pointer', borderRadius: '10px', fontWeight: '700' }}>
      ...
      {showForm && (
        <div style={{ ...card, padding: '32px', marginBottom: '24px' }}>
          <div style={{ fontFamily: 'var(--font-inter)', fontSize: '10px', color: accent, letterSpacing: '0.16em', textTransform: 'uppercase' as const, marginBottom: '20px' }}>New Lesson</div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '14px', marginBottom: '14px' }}>
            ...
            <div>
              <label style={{ display: 'block', fontFamily: 'var(--font-inter)', fontSize: '9px', color: textMuted, letterSpacing: '0.1em', textTransform: 'uppercase' as const, marginBottom: '6px' }}>Order Number</label>
          ...
          <div style={{ marginBottom: '14px' }}>
            <label style={{ display: 'block', fontFamily: 'var(--font-inter)', fontSize: '9px', color: textMuted, letterSpacing: '0.1em', textTransform: 'uppercase' as const, marginBottom: '6px' }}>Lesson Title</label>
          ...
          <div style={{ marginBottom: '20px' }}>
            ...
            {form.video_url ? (
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '12px', background: 'rgba(34,197,94,0.06)', border: '0.5px solid rgba(34,197,94,0.2)', borderRadius: '10px' }}>
                ...
                <div style={{ flex: 1 }}>
                  <div style={{ fontFamily: 'var(--font-inter)', fontSize: '11px', color: '#22c55e', fontWeight: '600' }}>Video uploaded successfully</div>
                  <div style={{ fontFamily: 'var(--font-inter)', fontSize: '10px', color: textMuted, marginTop: '2px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' as const }}>{form.video_url}</div>
                </div>
              ...
              >
                ...
                <span style={{ fontFamily: 'var(--font-inter)', fontSize: '12px' }}>{uploading ? `Uploading... ${uploadProgress}%` : 'Click to upload video'}</span>
                <span style={{ fontFamily: 'var(--font-inter)', fontSize: '10px', color: textMuted }}>MP4, MOV, WebM — large files supported</span>
              </button>
            )}
          ...
          <button onClick={saveLesson} disabled={saving || !form.title} style={{ background: accent, color: '#ffffff', fontFamily: 'var(--font-inter)', fontSize: '11px', letterSpacing: '0.1em', textTransform: 'uppercase' as const, padding: '12px 28px', border: 'none', cursor: saving || !form.title ? 'not-allowed' : 'pointer', borderRadius: '10px', fontWeight: '700', opacity: saving || !form.title ? 0.6 : 1 }}>
      ...
      {/* Lessons by course */}
      {loading ? (
        <div style={{ textAlign: 'center', padding: '60px', fontFamily: 'var(--font-playfair)', fontStyle: 'italic', color: textMuted }}>Loading...</div>
      ) : (
            ...
            return (
              <div key={course.id} style={{ ...card, overflow: 'hidden' }}>
                ...
                {courseLessons.length === 0 ? (
                  <div style={{ padding: '24px', fontFamily: 'var(--font-playfair)', fontStyle: 'italic', color: textMuted, textAlign: 'center' }}>No lessons yet</div>
                ) : (
