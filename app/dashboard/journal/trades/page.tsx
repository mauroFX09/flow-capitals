...
export default function TradeLog() {
  ...
  const [form, setForm] = useState({
    ...
    emotion: 'Calm',
    notes: '',
    followed_plan: null as boolean | null,
  })
  ...
  async function saveTrade() {
    ...
    if (!error) {
      setForm({ trade_date: today, pair: 'EUR/USD', direction: 'Long', entry_price: '', stop_loss: '', take_profit: '', lot_size: '', pnl: '', emotion: 'Calm', notes: '', followed_plan: null })
      setScreenshots([])
      setShowForm(false)
      loadTrades(userId)
    }
    ...
    <div style={{ padding: '40px 48px', background: bg, minHeight: '100vh' }}>
      ...
      {showForm && (
        <div style={{ ...card, padding: '32px', marginBottom: '20px' }}>
          <div style={{ fontFamily: 'Arial, sans-serif', fontSize: '10px', color: accent, letterSpacing: '0.16em', textTransform: 'uppercase' as const, marginBottom: '24px' }}>New Trade Entry</div>
          ...
          <>
            <div style={{ overflowX: 'auto' as const }}>
              <table style={{ width: '100%', borderCollapse: 'collapse' as const }}>
                ...
                <tbody>
                  {paginated.map(trade => (
                    <tr key={trade.id} style={{ borderBottom: `0.5px solid ${tableBorder}` }}
                      onMouseEnter={e => e.currentTarget.style.background = dark ? 'rgba(255,255,255,0.03)' : 'rgba(43,94,167,0.03)'}
                      onMouseLeave={e => e.currentTarget.style.background = 'transparent'}
                    >
                      {/* Date */}
                      <td style={{ padding: '12px 14px', fontFamily: 'Arial, sans-serif', fontSize: '11px', color: textMuted, whiteSpace: 'nowrap' as const }}>
                      ...
                      {/* Entry */}
                      <td style={{ padding: '12px 14px', fontFamily: 'Arial, sans-serif', fontSize: '12px', color: textMuted }}>
                        {trade.entry_price || '—'}
                      </td>
                      {/* R:R */}
                      <td style={{ padding: '12px 14px', fontFamily: 'Arial, sans-serif', fontSize: '12px', color: textMuted }}>
                      ...
                      {/* P&L */}
                      <td style={{ padding: '12px 14px', fontFamily: 'Georgia, serif', fontSize: '13px', fontWeight: '700', color: trade.pnl > 0 ? '#22c55e' : trade.pnl < 0 ? '#dc3232' : textMuted }}>
                          ...
                          <div style={{ display: 'flex', gap: '4px' }}>
                            {trade.screenshot_urls.slice(0, 3).map((url, i) => (
                              <a key={i} href={url} target="_blank" rel="noopener noreferrer">
                                <img src={url} alt="chart" style={{ width: '28px', height: '20px', objectFit: 'cover', borderRadius: '3px', border: `0.5px solid ${tableBorder}` }} />
                              </a>
                            ))}
                          </div>
                        ) : <span style={{ color: textMuted, fontSize: '11px' }}>—</span>}
